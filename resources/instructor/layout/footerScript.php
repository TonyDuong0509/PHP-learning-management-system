<script src="<?php ABSPATH ?>/public/backend/js/bootstrap.bundle.min.js"></script>
<!--plugins-->
<script src="<?php ABSPATH ?>/public/backend/js/jquery.min.js"></script>
<script src="<?php ABSPATH ?>/public/backend/plugins/simplebar/js/simplebar.min.js"></script>
<script src="<?php ABSPATH ?>/public/backend/plugins/metismenu/js/metisMenu.min.js"></script>
<script src="<?php ABSPATH ?>/public/backend/plugins/perfect-scrollbar/js/perfect-scrollbar.js"></script>
<script src="<?php ABSPATH ?>/public/backend/plugins/vectormap/jquery-jvectormap-2.0.2.min.js"></script>
<script src="<?php ABSPATH ?>/public/backend/plugins/vectormap/jquery-jvectormap-world-mill-en.js"></script>
<script src="<?php ABSPATH ?>/public/backend/plugins/chartjs/js/chart.js"></script>
<script src="<?php ABSPATH ?>/public/backend/plugins/datatable/js/jquery.dataTables.min.js"></script>
<script src="<?php ABSPATH ?>/public/backend/plugins/datatable/js/dataTables.bootstrap5.min.js"></script>
<script src="<?php ABSPATH ?>/public/backend/js/index.js"></script>
<!--app JS-->
<script src="<?php ABSPATH ?>/public/backend/js/app.js"></script>
<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>